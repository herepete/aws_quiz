...
["What is a use case for Deeplens,Lambda,Secrets Manager and DynamoDB & Regonition?","\n An AWS DeepLens device enables you to run deep learning on the edge.\nIt detects an object and runs it against an object detection model. \nWhen the model detects a car, it uploads a frame to Amazon S3. \nWhen a new image is stored in an S3 bucket, it triggers an AWS Lambda function, which initiates a call to Amazon Rekognition to compare the license plate to a list of allowed values in an Amazon DynamoDB table. \nIf the function locates the license plate, it retrieves third-party API secrets from AWS Secrets Manager and triggers a third-party API to open the garage door.","Machine Learning"],
["Where might you use Transcribe,Translate and Polly together?","\nReal time audio transaltion say for a Doctor\nTranscribe -For speach to text,\nTranslate -deliver accurate translations \nPolly  text-to-speech service that can either send back lifelike audio clip responses for immediate streaming playback \nhttps://aws.amazon.com/blogs/machine-learning/break-through-language-barriers-with-amazon-transcribe-amazon-translate-and-amazon-polly/#:~:text=Amazon%20Polly%20converts%20the%20speech,Audio%20is%20outputted%20to%20speakers.","Machine Learning"],
