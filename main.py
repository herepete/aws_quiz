...
["Where would you use a RDS database over a EC2 Database?","RDS is\nhighly optimized,\nno dba needed,\nrds deals with backups\nrds deals with patching , \nyou deal with optimization and tuning rather than database admin","Database"],
[""," ","Database"],
...
["How could you Manage infrastructure with Devops?","AWS CloudFormation\n\nSpeed up cloud provisioning with infrastructure as code","Other"],
...
["What could help access control and accelerate proviosing of CI/CD pipelines?","Aws Service Catalog \n\nCreate, share, organize, and govern your curated IaC templates","Other"],
...
["What is the difference between Amazon connect and direct connect","Connect -  service that operates as a Cloud based call centre \nDirect Connect -  you can maintain a dedicated connection between AWS and your on-premise.","Other"],
...
["What is a use case for Deeplens,Lambda,Secrets Manager and DynamoDB & Regonition?","\n\nLets imagine a Smart Garage door which we only want to open for Valid Cars....\n\nDeeplens - When a car is detected, \nS3 -it uploads a frame to Amazon S3. \nLamnda - When a new image is stored in an S3 bucket, it triggers an AWS Lambda function, \nRekognition - which initiates a call to Amazon Rekognition to compare the license plate to a list of \nDynamoDB - allowed values in an Amazon DynamoDB table. \nAWS Secrets - If the function locates the license plate, it retrieves third-party API secrets from AWS Secrets Manager and triggers a third-party API to open the garage door.\n\nhttps://aws.amazon.com/blogs/machine-learning/building-a-smart-garage-door-opener-with-aws-deeplens-and-amazon-rekognition/","Machine Learning"],
["Where might you use Transcribe,Translate and Polly together?","\nReal time audio transaltion say for a Doctor\n\nTranscribe - For speech to text,\nTranslate - deliver accurate translations \nPolly  text-to-speech service that can either send back lifelike audio clip responses for immediate streaming playback \n\nhttps://aws.amazon.com/blogs/machine-learning/break-through-language-barriers-with-amazon-transcribe-amazon-translate-and-amazon-polly/#:~:text=Amazon%20Polly%20converts%20the%20speech,Audio%20is%20outputted%20to%20speakers.","Machine Learning"],
...
["Which OS is the CLI installed on by default?","Linux AMI not unbuntu ","CLI"],
...
["Which 4 main areas in which AWS customer are realizing value by moving to cloud computing?","\nCost Saving\nStaff Productivity\nOperational Resilience\nBuisness Agility","Buisness Case"],
...
["Which sla should you choose if you have production workloads in aws?","Business","Buisness Case"],
["Which sla should you choose if you are business or mission Critical workloads ?","Enterprise","Buisness Case"],
["What does the Concierge support team do ?","help with billing and account inquries & related best practices","Buisness Case"],
["How can you get an idea on cost savings for using AWS ?","Aws pricing caculator","Buisness Case"],
["What can help you create a data driven business case for migration to aws ?","Aws Migration Evaluator","Buisness Case"],
["What can test your adoption readiness to aws ?","Aws cloud adoption Readiness tool (CART)","Buisness Case"],
["What would allow you to access,audit and evaliate the config of your aws resources ?","Aws config","Buisness Case"],
["What 2 tools could help if you are working in a security team to help with compliance ?","Aws compliance program & aws config","Buisness Case"],
["How can you esimtate all expenses in purchasing and operating equipment over its life cycle ?","Aws total cost of ownership (TCO)","Buisness Case"],
...
["What are Loosley coupled Applications?","A loosely coupled workload entails the processing of a large number of smaller jobs. Generally, the smaller job runs on one node, either consuming one process or multiple processes with shared memory parallelization (SMP) for parallelization within that node.\n\nLoosely coupled applications are found in many areas, including Monte Carlo simulations, image processing, genomics analysis, and Electronic Design Automation (EDA).","Arch and Compilance"],
...
["What service enables users to monitor for specific phrases, value or patterns and set up alarms based on metrics? ","Amazon Cloudwatch logs","Cloudwatch"],
...
["Which 2 services can be used to block network traffic to an instance","\n1)Security Groups-Act as a Firewall for associated EC2 instaces controlling both inbound and outbound traffic at the instance level\n2)Network ACL-control inbound and outbound traffic at the subnet level\nboth act in effect as firewalls","Deployment"],
...
["Which Service can run a managed PostgreSQL database?","RDS","Database"],
["What are 2 benefits over using RDS over EC2 for relational datbases?","\n1)Automated backups\n2)Software Patching","Database"],
["If you wanted to move a EC2 database to a higher avaliability solution. What should you consider?","Migrate to RDS and enable Multi-AZ","Database"],
["What is the customer responsability for RDS?","Security","Database"],
["When using RDS according to the shared responsabliity model who is respoisble for Scheduling and performing backups?","Sceduling -customer, performing -AWS","Database"],
["Which service has the ability to host a nosql database?","DynamoDB","Database"],
["What could you use to consolidate data into one data Warehouse?","Redshift","Database"],
["What could you use to process a large amount of data by making graphical queries with High throughput?","Neptune","Database"],
["Which acts as a data extra , transform and load (ETL) tool to make it easy to prepare data for analtics?","AWS Glue","Database"],
["What could you use for dashboards and reporting services?","Amazon quicksight","Database"],
...
["How should a EC2 instance be given access to an S3 bucket?","Create an IAM profile that grants access to S3","S3"],
...
["AWS Trusted Advisor","Reduce costs, improve performance, improve security by providing recommendations that help you follow AWS best practices "],
        ...
        for all in questions:
            try:
                ...
                if all[2] == user_input_section_choice_word:
                    ...
                    print()
                    print()
                    ui3=input("....press enter to continue.......")
                    if ui3=="b":
                        breakpoint()
